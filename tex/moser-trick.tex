...
\[\vec v_t(\vec x)=-\phi(t\cdot(1-|\vec x|))\cdot f(\vec x)\cdot \vec x.\]
Let $\phi^s$ be the corresponding flow.
...
We plan to find a vector field $\vec v_t$ such that for its flow $\phi^t$ the value $f_t\circ \phi^t(x)$ does not depend on $t$ (while it is defined) and $\vec v_t(p)=0$ for any $t$.
